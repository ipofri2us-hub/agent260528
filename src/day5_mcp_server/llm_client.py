...
# llm.mock_llm / llm.local_llm / llm.cloud_llm 은 src/ 하위 패키지이므로
# 이 파일의 위치(src/day5_mcp_server/)에서 src/ 를 sys.path 에 추가한다.
...
def _mask_sensitive_text(text: str) -> str:
    ...
    # 주의: "{24,}" 같이 길이만으로 마스킹하는 패턴은 arXiv ID·UUID·긴 영문 문장까지
    # 마스킹해 에러 메시지를 판독 불능으로 만들므로 사용하지 않는다.
    return masked
